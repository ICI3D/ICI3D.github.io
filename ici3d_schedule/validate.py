...
Two gating tiers run here; Tier 2 (link-liveness) is a separate advisory job and is NOT run here.
  ...
  Tier 0 - schema:    JSON Schema (ici3d_schedule/data/schedule-cohort.schema.json). Required-fields-per-kind,
...
Usage (installed console script, or via the repo shim tools/validate_schedule.py):
    validate-schedule _data/schedule/mmed/2025.yml [more.yml ...] \
        [--schema ...] [--roles ...]  (both default to this package's bundled copies) \
        [--people-dir _data/team]     (defaults to _data/team in the current repo) \
        [--github]

The JSON Schema and the role-token list ship *inside this package* (ici3d_schedule/
data/), so a clinic repo that pins the pre-commit hook validates against the shared,
versioned schema without vendoring it. People records (--people-dir) are clinic-owned
and resolved from the working repo, matching the renderer's site.data.team lookup.
...
# Schema + role tokens ship as package data (ici3d_schedule/data/), one dir up-tree
# from this module. This path resolves identically whether the package is pip-installed
# (site-packages/ici3d_schedule/data) or run from a repo checkout, so the distributed
# pre-commit hook and the in-repo script share one source of truth.
...
def main(argv=None):
    ...
    ap.add_argument("--schema", default=str(_PKG_DATA / "schedule-cohort.schema.json"))
    ap.add_argument("--people-dir", default="_data/team",
                    help="clinic-owned people records (default: _data/team in the current repo)")
    ap.add_argument("--roles", default=str(_PKG_DATA / "roles.yml"))
